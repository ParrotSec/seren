 ...
 * http://link.springer.com/chapter/10.1007/3-540-58108-1_24
 */
...
/* round function */
#define F(x) (((ctx->S1[A(x)] + ctx->S2[B(x)]) ^ ctx->S3[C(x)]) + ctx->S4[D(x)])

#ifdef SMALL
static void blowfish_encipher(const struct blowfish_ctx *ctx, uint32_t *l, uint32_t *r)
{
	uint32_t i, l0, r0;
...
static void blowfish_decipher(const struct blowfish_ctx *ctx, uint32_t *l, uint32_t *r)
{
	uint32_t i, l0, r0;
...
void blowfish_init(struct blowfish_ctx *ctx, const uint8_t key[], size_t keylen)
{
	...
	ctx->keylen = ctx->keylen >= 448 ? 448 : ctx->keylen;
	memcpy(ctx->key, key, ctx->keylen / 8);
	ctx->keylen = ctx->keylen <=  32 ?  32 : ctx->keylen;
	...
	for (i = 0; i < 18; i++) {
		...
		for (h = 0; h < 4; h++) {
			k32 = (k32 << 8) | ctx->key[j++];
			j = j >= (ctx->keylen >> 3) ? 0 : j;
		}
		ctx->P[i] = Pi[i] ^ k32;
	}

	memcpy(ctx->S1, S1i, sizeof(ctx->S1));
	memcpy(ctx->S2, S2i, sizeof(ctx->S2));
	memcpy(ctx->S3, S3i, sizeof(ctx->S3));
	memcpy(ctx->S4, S4i, sizeof(ctx->S4));
...
int main(int argc, char *argv[])
{
	...
	const uint8_t   ct[8] = {0xA8, 0x10, 0x30, 0x5C, 0xEC, 0xB0, 0x6D, 0xB5};
	uint8_t        buf[8];
	int cmp_enc, cmp_dec;

	fprintf(stderr, "Blowfish selftest\n");
	memcpy(buf, pt, sizeof(pt));
	blowfish_init(&ctx, key, 128);
	...
	blowfish_encrypt_buffer_ecb(&ctx, buf, 8);
	fprintf(stderr, "        ciphertext  = %08X%08X\n", read_be32(buf), read_be32(buf+4));
	cmp_enc = memcmp(buf, ct, 8);
	blowfish_decrypt_buffer_ecb(&ctx, buf, 8);
	fprintf(stderr, "        plaintext   = %08X%08X\n", read_be32(buf), read_be32(buf+4));
	...
	fprintf(stderr, "  run = %zd, time = %.3fs, speed = %.2f Mb/s\n", \
		...
		if (buf) {
			...
			for (i = 0; i < RUNS; i++) {
				...
				blowfish_encrypt_buffer_ecb(&ctx, buf, BUFDIM);
			...
			for (i = 0; i < RUNS; i++) {
				gettimeofday(&t0,  NULL);
				blowfish_decrypt_buffer_ecb(&ctx, buf, BUFDIM);
				gettimeofday(&t1,  NULL);
				PRINT_INTERVAL;
			}
			free(buf);
